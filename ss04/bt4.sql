...
HAVING so_luong IN (
SELECT MAX(so_luong) 
FROM (SELECT s.student_name,COUNT(s.id_student) AS so_luong 
FROM students s
JOIN borrows b ON s.id_student = b.id_student
GROUP BY s.id_student) bt
);
